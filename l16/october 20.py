...
try:
    devices=AudioUtilies.GetSpeakers()
...
if not cap.isOpened():
    print("Error :webcam not assecible")
    exit()
...
while True:
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        for i, handLms in enumerate(results.multi_hand_landmarks):
            label= results.multi_handedness[i].classfication[0].label
            mp_draw.draw_ladmarks(img,handLms,mp+hands.HAND_CONNECTIONS)
            ...
            cv2.line(img,thumb_pos,index_pos,(0,255,0),cv2,3)
            ...
            if label=="Right":
                ...
                try:
                    volume.SetMasterVolumeLLevel(vol,None)
                except Exception as e:
